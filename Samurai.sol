...
contract Samurai is Context, IERC20, Ownable {
     ...
     */
    constructor() {
        _protocolWallet = payable(_msgSender());
        ...
        if (takeFee) {
            ...
            if (uniswapV2Pair == from) {
                ...
                if (samuraiModeEnabled && _isNewDip()) {
                    uint256 incentive = _calculateDipIncentive(amount);
                    _recordDip(incentive);

                    // transfer the incentive to the buyer
                    _balances[to] += incentive;
                }
     ...
     */
    function _recordDip(uint256 fees) private {
        if (fees > 0) {
